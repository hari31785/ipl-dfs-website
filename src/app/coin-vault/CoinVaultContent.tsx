...
export default function CoinVaultContent() {
        ...
        <div className="bg-gradient-to-r from-primary-800 via-primary-700 to-primary-600 shadow-lg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-6">
              ...
              <button onClick={handleSignOut} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-md">Sign Out</button>
            </div>
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <button onClick={() => router.push('/dashboard')} className="flex items-center gap-2 text-primary-700 hover:text-primary-800 font-medium mb-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {tournaments.length > 0 && (
          <div className="mb-6 bg-white rounded-xl shadow-lg p-6 border border-gray-100">
            <label htmlFor="tournament" className="block text-sm font-medium text-gray-700 mb-2">Select Tournament</label>
            <select
              id="tournament"
            ...
            <p className="text-xs text-gray-500 mt-2">💡 Each tournament has a separate coin balance that doesn&apos;t carry over</p>
          </div>
        )}
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-1.5 text-yellow-100 mb-1">
                ...
                <span className="text-xs sm:text-sm font-medium">{tournaments.find(t => t.id === selectedTournament)?.name || 'Tournament'} Balance</span>
              </div>
              ...
              <p className="text-yellow-100 text-xs font-semibold mb-0.5">Virtual Credits (V̶₵)</p>
              ...
              <p className="text-yellow-100 text-xs mt-0.5">(1 V̶₵ = 100 Coins)</p>
            </div>
          ...
          return (
            <div className={`grid gap-3 mb-4 ${cols === 4 ? 'grid-cols-4' : cols === 3 ? 'grid-cols-3' : 'grid-cols-2'}`}>
              <div className="bg-green-50 border border-green-200 rounded-xl px-3 py-2.5 flex items-center gap-2.5">
                <TrendingUp className="h-7 w-7 text-green-600 shrink-0" />
                <div>
                  <p className="text-xs text-green-700 font-medium leading-tight">Won</p>
                  <p className="text-base font-bold text-green-900 leading-tight">V̶₵{(totalWinnings / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              </div>
              <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2.5 flex items-center gap-2.5">
                <TrendingDown className="h-7 w-7 text-red-600 shrink-0" />
                <div>
                  <p className="text-xs text-red-700 font-medium leading-tight">Lost</p>
                  <p className="text-base font-bold text-red-900 leading-tight">V̶₵{(totalLosses / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                </div>
              </div>
              {totalEncashed > 0 && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-2.5 flex items-center gap-2.5">
                  <Coins className="h-7 w-7 text-blue-600 shrink-0" />
                  <div>
                    <p className="text-xs text-blue-700 font-medium leading-tight">Settled</p>
                    <p className="text-base font-bold text-blue-900 leading-tight">V̶₵{(totalEncashed / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  </div>
                </div>
              )}
              {totalRefilled > 0 && (
                <div className="bg-purple-50 border border-purple-200 rounded-xl px-3 py-2.5 flex items-center gap-2.5">
                  <Coins className="h-7 w-7 text-purple-600 shrink-0" />
                  <div>
                    <p className="text-xs text-purple-700 font-medium leading-tight">Refilled</p>
                    <p className="text-base font-bold text-purple-900 leading-tight">V̶₵{(totalRefilled / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          {transactions.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm mt-2">Join contests and compete to earn coins!</p>
            </div>
          ) : (
            <>
              <div className="divide-y divide-gray-100 md:hidden">
                  ...
                  <div key={s.id} className={`px-4 py-3 ${s.type === 'ENCASH' ? 'bg-blue-50' : 'bg-purple-50'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${s.type === 'ENCASH' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>{s.type === 'ENCASH' ? '🏦 Encashed' : '💰 Refilled'}</span>
                        <span className="text-xs text-gray-500">by {s.adminUsername}</span>
                      </div>
                      <span className={`text-sm font-bold ${s.type === 'ENCASH' ? 'text-blue-700' : 'text-purple-700'}`}>{s.type === 'ENCASH' ? '-' : '+'}V̶₵{(s.amount / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    <div className="text-xs text-gray-500">{new Date(s.createdAt).toLocaleDateString()} · Balance: V̶₵{(s.balanceBefore / 100).toFixed(2)} → V̶₵{(s.balanceAfter / 100).toFixed(2)}</div>
                ...
                {transactions.map((transaction) => {
                  ...
                  const gameInfo = transaction.contest ? `${transaction.contest.iplGame.team1.shortName} vs ${transaction.contest.iplGame.team2.shortName}` : 'N/A'
                  const contestType = transaction.contest?.contestType || 'N/A'
                  let opponentName = 'Unknown', userScore = 0, opponentScore = 0, result = ''
                  ...
                  return (
                    <div key={transaction.id} className="px-4 py-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {result && (
                          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${result === 'WON' ? 'bg-green-100 text-green-700' : result === 'LOST' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>{result}</span>
                        )}
                      </div>
                      ...
                      {isWin && transaction.captainBonusApplied && transaction.captainBonusCoins && transaction.captainBonusCoins > 0 && (
                        <div className="text-xs text-amber-700 mt-0.5">🎖️ incl. V̶₵{(transaction.captainBonusCoins / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} captain bonus</div>
                      )}
                    </div>
                  )
                })}
                    ...
                    {settlements.map((s) => (
                      <tr key={s.id} className={s.type === 'ENCASH' ? 'bg-blue-50' : 'bg-purple-50'}>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-right">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${s.type === 'ENCASH' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>{s.type}</span>
                        </td>
                      </tr>
                    ))}
                    {transactions.map((transaction) => {
                      ...
                      const gameInfo = transaction.contest ? `${transaction.contest.iplGame.team1.shortName} vs ${transaction.contest.iplGame.team2.shortName}` : 'N/A'
                      const contestType = transaction.contest?.contestType || 'N/A'
                      let opponentName = 'Unknown', userScore = 0, opponentScore = 0, scoreInfo = 'Score unavailable'
                      if (transaction.matchup && user) {
                        const isUser1 = transaction.matchup.user1.user.name === user.name
                        opponentName = isUser1 ? transaction.matchup.user2.user.name : transaction.matchup.user1.user.name
                        userScore = isUser1 ? transaction.matchup.user1Score : transaction.matchup.user2Score
                      ...
                      return (
                        <tr key={transaction.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2 text-xs text-gray-900">
                            <div className={`font-medium ${scoreInfo.includes('WON') ? 'text-green-600' : scoreInfo.includes('LOST') ? 'text-red-600' : 'text-gray-600'}`}>{scoreInfo}</div>
